...
def get_sinusoid_encoding_table(n_position, d_model):
    '''
    位置
    传入的参数分别是字典的大小，以及位置编码的维度。最终返回的参数，维度也即[n_position, d_model]，和字编码的大小一摸一样
    '''
...
def get_attn_pad_mask(seq_q, seq_k):
    ...
    try:
        ...
        batch_size,len_k = seq_k.size(0),50
        pad_attn_mask = torch.full((batch_size,1,50),False).cuda()#全部填充为False
        pad_attn_mask.fill_(False)
...
def get_attn_subsequence_mask(seq):
    '''
    decoder中用到，主要作用是屏蔽未来时刻单词的信息。首先通过np.ones()生成一个全1的方阵，然后通过np.triu()生成一个上三角矩阵
